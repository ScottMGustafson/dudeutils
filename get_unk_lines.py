...
c=299792.458
xmlfile="varied_cont_metals.xml"
unk_line=3896.7
vel_tol=10.
...
def check_DH(line1, line2, vel_sep=81.55, vel_tol=5.0, dh=-4.6, dh_tol=0.5):
    if vel_sep-vel_tol<=c*(line2.z-line1.z)/(1.+line1)<=vel_tol+vel_sep:
        if np.fabs(line2.N-line1.N-dh)<=dh_tol:
            return True
    return False

if __name__ == "__main__":
    model=newdb(xmlfile,chi2=0.,pixels=0.,params=0.)[0]
    ...
    for item in candidates:
        print("%6s:   rest=%6.2lf     obs=%6.2lf    z=%10.8lf   vel=%  5.3lf   N=%5.3lf"%(item["name"], item["rest"], obs, item["z"],item["vel"],item["N"]))
