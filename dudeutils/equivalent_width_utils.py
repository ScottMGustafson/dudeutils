...
def get_maxN(ab, trans, continuum_level, SNR, flux, err, nsigma=2., vdisp=2.14):
    ...
    print("ew=%lf+/-%lf Angstroms" % (
    10. ** 10 * W * rest_lmda * 1000. * vdisp / c, 10. ** 10 * dW * rest_lmda * 1000. * vdisp / c))
...
if __name__ == "__main__":
    ...
    for key, val in dct.items():
        ...
        for item in val:
            ...
            print("%s %s logNmax=%lf \n" % (key, ab.get_rest_wave(item['trans']),
                                            get_maxN(ab, item['trans'], cont, SNR, spec.flux.tolist(),
                                                     spec.error.tolist())))
